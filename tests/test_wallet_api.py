...
def test_wallet_balance_success(monkeypatch):
    ...
    # Mock pricing_config table select (returns single row with usd_per_credit)
    mock_db.table().select().eq().limit().execute.return_value.data = [
        {"usd_per_credit": 0.01}
    ]

    # Mock users table select (returns credit_balance)
    def mock_table(table_name):
        mock_table_obj = MagicMock()
        if table_name == "pricing_config":
            # First call: get usd_per_credit
            mock_table_obj.select().eq().limit().execute.return_value.data = [
                {"usd_per_credit": 0.01}
            ]
        elif table_name == "users":
            # Second call: get credit_balance
            mock_table_obj.select().eq().limit().execute.return_value.data = [
    ...
    data = response.json()
    assert data["credit_balance"] == 1000.0
    assert data["usd_balance"] == 10.0  # 1000 * 0.01
...
def test_wallet_packages_success(monkeypatch):
    ...
    # Mock topup_packages table select with order
    packages = [
        {"id": "pkg-1", "name": "Small", "credit_amount": 100, "usd_price": 1.0},
        {"id": "pkg-3", "name": "Large", "credit_amount": 1000, "usd_price": 8.0},
        {"id": "pkg-2", "name": "Medium", "credit_amount": 500, "usd_price": 4.0},
    ]

    def mock_table(table_name):
        mock_table_obj = MagicMock()
        if table_name == "topup_packages":
            # Simulate order by price
    ...
    prices = [p["usd_price"] for p in data["packages"]]
